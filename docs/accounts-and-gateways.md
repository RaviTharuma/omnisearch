...
Pass optional `account` (MCP/HTTP tool param) to pin a named credential instead of rotating. Example: `account=work` on `search`, single-provider tools (`tavily_search`, `brave_search`, …), `extract`, `research`, and Firecrawl `scrape`/`crawl`/`map`. When omitted, behavior stays round-robin with failover. Unknown account names **fail closed** (invalid request / provider failure); credentials are never echoed.

### Priority providers

Linkup, Exa, Tavily, Brave, and Firecrawl all support named accounts. Firecrawl `crawl`/`map` use the same account pool as search/extract (accounts-only setups work; no legacy key required). Linkup search accepts optional `depth` (`fast` | `flash` | `standard` | `deep`, default `standard`), sends `maxResults` from the page-size hint, maps freshness to `fromDate`, and extracts via `POST /v1/fetch`.

### Social priority providers

X, Reddit, and Discord are first-class social search targets with the same named-account pool:

- **X** — `{ "bearer_token": "..." }` and/or `{ "xai_api_key": "..." }` accounts; legacy `X_BEARER_TOKEN` / `XAI_API_KEY` rings when no accounts are set.
- **Reddit** — `{ "client_id": "...", "client_secret": "..." }` for OAuth pools, or `{ }` for public `search.json` (always available without keys).
- **Discord** — `{ "token": "...", "guild_ids": ["..."] }` (optional `"bearer": true` for user tokens). Legacy: `DISCORD_BOT_TOKEN` + comma-separated `DISCORD_GUILD_IDS`. Guild-scoped `GET /guilds/{id}/messages/search` only — Discord has no public global search API, and Bot tokens are often rejected on this route (use a user bearer or expect skip/failure).

Pool admission uses the highest estimated search cost across its accounts and the lowest supported page-size limit, independent of account order. This also applies to OmniRoute gateway pools. With `OMNISEARCH_AUTO_ALLOW_USD=0`, automatic selection skips an entire mixed free/paid pool rather than risking paid rotation or failover. Explicit provider selection retains its existing cost-gate bypass.

Each account contains `provider`, `name`, and a `credentials` object. Credential shapes: `{ "api_key": "..." }` for key-based adapters; `{ "bearer_token": "..." }` or `{ "xai_api_key": "..." }` for X; `{ "client_id": "...", "client_secret": "..." }` or `{ }` for Reddit; `{ "token": "...", "guild_ids": ["..."], "bearer": false }` for Discord; `{ "token": "...", "user_id": "..." }` for Instagram; `{ "token": "..." }` for Facebook; `{ "instance": "https://mastodon.example", "token": "..." }` for Mastodon; `{ "url": "https://mcp.example/mcp", "token": "..." }` for MCP backends. Public Reddit/Wikipedia/Bluesky (and Scholar) use `{}`. Unsupported combinations are rejected rather than silently ignored. Endpoints remain provider-level configuration. Provider aliases and legacy key names remain unchanged.

Supported existing authentication only: Reddit client ID + client secret uses its existing OAuth client-credentials flow; Reddit bearer-token injection is not implemented. X uses bearer tokens or existing xAI API keys. Discord uses bot or bearer tokens you already hold (no browser OAuth/login flow). Mastodon uses instance URL + optional bearer token; Bluesky currently uses public search; unused legacy login variables are not promoted to a working authentication flow. Other adapters use their existing API-key/token mechanism. No new OAuth browser-login or refresh-token implementation is claimed. Never put real credentials in source control.

## Official MCP remotes

`OMNISEARCH_MCP_BACKENDS=official` attaches vendor MCP remotes for every configured key, not only the first env var. Legacy rings expand as `tavily`, `tavily_2`, `tavily_3`, and the same for Exa, Firecrawl, Linkup, Kagi, and Perplexity. When `OMNISEARCH_ACCOUNTS` defines `api_key` accounts for those providers, the official preset uses those named accounts instead (backend names like `tavily-work`) and ignores the replaced legacy keys for that provider — the same replacement rule as native adapters. Brave is not part of the official remote preset; it remains a must-have default native fan-out provider when keyed.
