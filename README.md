...
**MCP server** for **parallel web search** across many engines — so your AI finds sources a single search (or the model alone) usually misses.

Plug it into Claude, Cursor, or any [MCP](https://modelcontextprotocol.io) client. Drop in the API keys you already have.

**Why people use it**

- **It’s an MCP** — one server, standard tools, works where your agents already run
- **Parallel by default** — several search engines at once, not one at a time
